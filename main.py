...
class BlazeFace():
    ...
    def build_model(self):
        ...
        bb_16_conf_reshaped = tf.keras.layers.Reshape(
            (16**2 * self.n_boxes[0], 1))(bb_16_conf)
        ...
        bb_8_conf_reshaped = tf.keras.layers.Reshape(
            (8**2 * self.n_boxes[1], 1))(bb_8_conf)
        # Concatenate confidence prediction

        # shape : [batch_size, 896, 1]
        conf_of_bb = tf.keras.layers.Concatenate(axis=1, name='clf_output')(
            [bb_16_conf_reshaped, bb_8_conf_reshaped])
        ...
        bb_16_loc_reshaped = tf.keras.layers.Reshape(
            (16**2 * self.n_boxes[0], 4))(bb_16_loc)

        # 8x8 bounding box - loc [x, y, w, h]
        bb_8_loc = tf.keras.layers.Conv2D(filters=self.n_boxes[1] * 4,
                                          kernel_size=3,
                                          padding='same')(model.output[1])
        bb_8_loc_reshaped = tf.keras.layers.Reshape(
            (8**2 * self.n_boxes[1], 4))(bb_8_loc)
        # Concatenate  location prediction

        loc_of_bb = tf.keras.layers.Concatenate(axis=1)(
            [bb_16_loc_reshaped, bb_8_loc_reshaped])

        output_combined = tf.keras.layers.Concatenate(
            axis=-1, name='bb_output')([conf_of_bb, loc_of_bb])
    ...
    def train(self):
        ...
        data_gen = dataloader(config.dataset_dir,
                              config.label_path, self.batch_size)
        ...
        for epoch in range(self.nb_epoch):
            ...
            if epoch % 100 == 0:
                model.save_weights(os.path.join(
                    config.checkpoint_path, str(epoch)))
...
if __name__ == "__main__":
    ...
    args.add_argument('--numdata', type=int, default=2989)
