...
def smooth_l1_loss(true, pred):
    ...
    lossx = face_label * \
        tf.map_fn(smooth_l1, tf.reshape(gxs - logitx, (-1, 1)))
    lossy = face_label * \
        tf.map_fn(smooth_l1, tf.reshape(gys - logity, (-1, 1)))
    lossw = face_label * \
        tf.map_fn(smooth_l1, tf.reshape(gws - logitw, (-1, 1)))
    lossh = face_label * \
        tf.map_fn(smooth_l1, tf.reshape(ghs - logith, (-1, 1)))
